...
def plot_series(plot, show_plot_window):
    ...
    if not data.empty:
        ...
        step_index = data[
            data[list(plot_settings["columns_to_plot"])[0]] != data[list(plot_settings["columns_to_plot"])[0]].shift()
        ].index[1]
        ...
        if plot.get("secondary_axis", {}).get("show", True):
            ...
            print(f"    PI parameters (SIMC): Kp={T/(2*K*tau)} bit/K, Ki={T/(2*K*tau)/np.minimum(T, 8*tau)} bit/(Ks)")
            print(
